	  ...
	  function(data, menu)
		...
		if player ~= -1 and distance <= 3.0 then
		  ...
		  if amount == nil then
			...
			Citizen.Wait(5000)
			  TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_kfc', ('kfc'), amount)
			  Citizen.Wait(100)
...
RMenu.Add('kaito0511', 'main', RageUI.CreateMenu("KFC", "Intéraction..."))
RMenu.Add('kaito0511', 'inter', RageUI.CreateMenu("KFC", "Intéraction..."))
        ...
        RageUI.IsVisible(RMenu:Get('kaito0511', 'main'), true, true, true, function()
			...
			if onservice then
				...
				RageUI.ButtonWithStyle("Annonce Ouverture",nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
					if Selected then       
						TriggerServerEvent('AnnonceOuvertkfc')
					end
				end)
		
				RageUI.ButtonWithStyle("Annonce Fermeture",nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
					if Selected then      
						TriggerServerEvent('AnnonceFermerkfc')
					end
				end)

				RageUI.ButtonWithStyle("Annonce Recrutement", "Pour annoncer des recrutements au KFC", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
					if (Selected) then   
					TriggerServerEvent('KKFC:annoncerecrutementkfc')
				end
				end)
				...
				RageUI.ButtonWithStyle("Message aux Employés", "Pour écrire un message aux Employés", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
					...
					TriggerServerEvent('kaitoo:kfcjob', info, message)
				end
				end)
...
Citizen.Wait(0)
    ...
    RageUI.Visible(RMenu:Get('kaito0511', 'main'), not RageUI.Visible(RMenu:Get('kaito0511', 'main')))
		...
		function demenotter()
            ...
            TriggerServerEvent('KKFC:requestrelease', target_id, playerheading, playerCoords, playerlocation)
            Wait(5000)
			TriggerServerEvent('KKFC:handcuff', GetPlayerServerId(closestPlayer))
		else
		...
		Citizen.CreateThread(function()
			...
			local kfcmap = AddBlipForCoord(347.15, -885.08, 28.63)
			SetBlipSprite(kfcmap, 570)
			SetBlipColour(kfcmap, 1)
            SetBlipScale(kfcmap, 0.7)
			SetBlipAsShortRange(kfcmap, true)
	
			BeginTextCommandSetBlipName('STRING')
			AddTextComponentString("KFC")
			EndTextCommandSetBlipName(kfcmap)
...
RMenu.Add('garageKaito', 'main', RageUI.CreateMenu("Garage", "Garage du KFC...."))
...
Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garageKaito', 'main'), true, true, true, function() 
            ...
            RageUI.ButtonWithStyle("Sortir un Benson", "Vous sortez un camion...", {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
            if (Selected) then
            ESX.ShowAdvancedNotification("Garagiste Wendy", "Le camion arrive mon reuf !..", "", "CHAR_MINOTAUR", 1) 
            Citizen.Wait(2000)   
            spawnuniCar("benson")
            ESX.ShowAdvancedNotification("Garagiste Wendy", "Par contre fait gaffe gros !", "", "CHAR_MINOTAUR", 1) 
                    ...
                    RageUI.ButtonWithStyle("Mettre le camion dans le garage", "Pour ranger le camion.", {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
                        ...
                        if dist4 < 4 then
                            ESX.ShowAdvancedNotification("Garagiste Wendy", "Le camion est de retour merci!", "", "CHAR_MINOTAUR", 1)
                            DeleteEntity(veh)
                        end 
                        end
                        end)   
...
RegisterNetEvent('kaitoo:kfcjob')
AddEventHandler('kaitoo:kfcjob', function(service, nom, message)
	if service == 'patron' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('INFO KFC', '~b~A lire', 'Patron: ~g~'..nom..'\n~w~Message: ~g~'..message..'', 'CHAR_MINOTAUR', 8)
		Wait(1000)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "a_f_y_vinewood_04", 371.93, -827.56, 28.29, 131.97,  true, true, true) --"a_f_y_vinewood_04" ped a modif: https://docs.fivem.net/docs/game-references/ped-models/
